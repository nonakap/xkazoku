...
#if defined(MACOS)			// ñ{ìñÇÕdosioÇ…Ç†ÇÈÇ◊Ç´ÅH xkazoku
static void cnvunixpath(char *path) {
...
static void pathadd(MIDIMOD mod, const char *path) {
	...
	if (path) {
#if 1	// xkazoku
		milstr_ncpy(pl.path, path, sizeof(pl.path));
#if defined(MACOS)
		cnvunixpath(pl.path);
#endif
		if (path[0]) {
			plusyen(pl.path, sizeof(pl.path));
		}
#else	// np2
		pl.path[0] = '\0';
...
static void settone(MIDIMOD mod, int bank, int argc, char *argv[]) {
	...
	if (name) {
#if 1	// xkazoku
		milstr_ncpy(name, argv[1], MAX_NAME);
#if defined(MACOS)
		cnvunixpath(name);
#endif
#else	// np2
